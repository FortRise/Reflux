// https://github.com/JaThePlayer/CelesteMappingUtils/blob/main/Helpers/MethodDiff.cs
using System;
...
internal class MethodDiff
{
    ...
    public IReadOnlyList<Instr> Instructions => instructions;
    ...
    public MethodDiff(MethodBase orig)
    {
        ...
        instructions = cloneDef.Definition.Body.Instructions.Select(instr => new Instr(ElementType.Unchanged, instr, null, new(0))).ToList();
        ...
        foreach (var hook in hooked)
        {
            ...
            try
            {
                ...
                instructions = CreateDiff(instructions, instrs, hook.ManipulatorMethod);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to apply IL hook {hook.ManipulatorMethod.GetID()}: {ex}");
            }
        }
    }
